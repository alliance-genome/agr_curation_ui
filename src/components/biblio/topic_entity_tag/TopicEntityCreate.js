    ...
    <Container fluid>
		  ...
		  setTopicSelectLoading(false);
              ...
              onChange={(selected) => {
                if (selected.length > 0) {
                  ...
                  handleRowChange(index, 'topicSelect', selectedCurie); // Trigger handleRowChange with the selected topic
                }
              }}
	  ...
	  <Col sm="1">
          ...
          <Col className="form-label col-form-label" sm="2">
            {renderView() === "list" ? (
              <Form.Control as="textarea" id={`entitytextarea-${index}`} value={row.entityText} disabled={disabledEntityList} onChange={(e) => {
                const updatedRows = [...rows];
                updatedRows[index] = { ...updatedRows[index], entityText: e.target.value };
                setRows(updatedRows);
              }} />
            ) : (
              row.topicSelect === speciesATP && (
                <AsyncTypeahead
                  ...
                  onChange={(selected) => {
                    ...
                    updatedRows[index] = {
                      ...updatedRows[index],
                      entityResultList: extractedStrings.map(specie => {
                        const match = specie.match(/(.+) (NCBITaxon:\d+)/);
                        if (match) {
                          return {
